...
mysql_query("CREATE  TABLE user (host VARCHAR(10),hostname VARCHAR(50),userid VARCHAR(50) ,name VARCHAR(50) UNIQUE,password VARCHAR(50),email VARCHAR(50), nickname VARCHAR(50) )",$con);
// insert fake data
...
foreach ($arr as $value)
{
  ...
  echo "email is : ".$email."\r\n";
  mysql_query(sprintf("insert into user values('%s','%s','%s','%s','%s','%s','%s')","true",$name,md5($email),$name,$name,$email,$name),$con);
  foreach ($family as $value)
  {
     $familyname=$name.$value;
     $familyemail=$familyname."@163.com";
     $i = mysql_query(sprintf("insert into user values('%s','%s','%s','%s','%s','%s','%s')","false",$name,md5($email),$familyname,$familyname,$familyemail,$familyname),$con);
     if ($i)
     {
      echo "insert into user succssful \r\n";
    }
    else
    {
